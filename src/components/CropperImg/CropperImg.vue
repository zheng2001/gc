<template>
  <el-dialog title="裁剪头像" width="600px" :visible.sync="cropperVisible" append-to-body>
    <el-container>
      <el-col :span="18">
        <div class="cropper">
          <!--组件vueCropper-->
  ...
  data() {
    return {
      ...
      option: {
        ...
        autoCropWidth: 200, // 默认⽣成截图框宽度
        autoCropHeight:200, // 默认⽣成截图框⾼度
  ...
  methods: {
    ...
    finish() {
      this.$refs.cropper.getCropBlob(async (blob) => {
        ...
        fd.append('file', file);
        axios.post('http://localhost:9090/gc/front/user/avatar/upload', fd, {headers: {'Content-Type': 'multipart/form-data'}})
            .then(res => {
              res = res.data;
              if (res.code === '200'){
